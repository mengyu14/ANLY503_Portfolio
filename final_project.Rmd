...
ggplot(intake_cond, aes(Intake.Condition,number, fill = Animal.Type))+
  ...
  geom_text(aes(label=number), position=position_dodge(width=0.9), vjust=-0.25, size=2)+
  mytheme+
...
ggplot(sex, aes(Sex,number, fill = state))+
  ...
  geom_text(aes(label=number), position=position_stack(vjust = 0.5),size=2.5)+
  mytheme+
  scale_fill_manual(values = val)+
